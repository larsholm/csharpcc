...
Status: complete. All six remote CI jobs pass with 267 tests per job in
[run 35533760396](https://github.com/larsholm/csharpcc/actions/runs/35533760396),
validating commit `b7347aaead1443e1c80c1e16f1a6d7c8ee161c1e` on
`larsholm/csharpcc:codex/net10-modernization-ci` on 2026-09-20.

## Local verification

- .NET SDK 10.0.111 on Linux: all 267 solution tests pass in Debug and Release,
  including generic parser compatibility coverage in both output modes.
...
Windows consumers set UTF-8 console output explicitly so the test transport preserves Unicode values
independently of the machine's console code page.

| Platform | Debug | Release |
| --- | --- | --- |
| Linux (`ubuntu-latest`) | Passed, 267 tests | Passed, 267 tests |
| Windows (`windows-latest`) | Passed, 267 tests | Passed, 267 tests |
| macOS (`macos-latest`) | Passed, 267 tests | Passed, 267 tests |

Every job also passed both seven-file bootstrap checks and all 14 isolated
embedded C# boundary checks. The working checkout contains the validated source;
the final documentation status was updated after the run completed.
...
## Warning policy and scope

Existing warnings from legacy output and the bootstrap remain visible;
handwritten nullable warnings and selected modern
